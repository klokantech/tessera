...
var getTileUrls = function(domains, host, path, tilePath, format, key) {
  ...
  domains.forEach(function(domain) {
    uris.push("http://" + domain + path + tilePath.replace("{format}", getExtension(format)).replace(/\/+/g, "/") + query);
  });
  ...
  app.get("/index.json", function(req, res, next) {
    ...
    return tilelive.load(uri, function(err, source) {
      ...
      return tessera.getInfo(source, function(err, info) {
        ...
        info.tiles = getTileUrls(domains, req.headers.host, path.dirname(req.originalUrl), tilePath, info.format, req.query.key);
        info.tilejson = "2.0.0";
