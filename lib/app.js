...
var getTileUrls = function(domains, host, path, tilePath, format) {
  domains = domains && domains.length > 0 ? domains : [host];

  var uris = [];
  domains.forEach(function(domain) {
    uris.push("http://" + domain + path + tilePath.replace("{format}", getExtension(format)).replace(/\/+/g, "/"));
  });
  ...
  app.get("/index.json", function(req, res, next) {
    ...
    return tilelive.load(uri, function(err, source) {
      ...
      return tessera.getInfo(source, function(err, info) {
        ...
        info.tiles = getTileUrls(domains, req.headers.host, path.dirname(req.originalUrl), tilePath, info.format);
        info.tilejson = "2.0.0";
